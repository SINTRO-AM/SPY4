...
def calculate_rolling_values_and_kpis(df):
    ...
    df.loc[conditions, 'Signal'] = 2

    
    # In allen anderen Fällen setze Signal auf 0 --> Kommentar sagt auf 0 aber code setzt auf -1
    df['Signal'].fillna(-2, inplace=True)

    # SignalB
    df['SignalB'] = np.where(df['Signal'].isin([-2, 0]), 2, 0)
...
def generate_shapes_for_signals(df):
    ...
    for i, row in df_copy.iterrows():
        if row['Signal'] == 0:
            ...
            shape_color = '#fc7c7c'  # Corrected the typo here
        else:
            shape_color = '#a5fc7c'
