...
results_df = pd.DataFrame({
    ...
    'SPY5': [annual_return_pf, annual_std_pf, sharpe_ratio_pf, max_DD_SPY3],
...
annual_returns_table = pd.DataFrame({
    ...
    'SPY5 Annual Returns': ["{:.2f}%".format(val * 100) for val in annual_returns_spy3.values]
})
...
heatmap_spy3 = create_heatmap(heatmap_data_spy3, "Monthly Returns of the SPY5 Model")
...
def update_graph(relayoutData):
    ...
    trace2 = go.Scatter(x=df['Date'], y=df['Total_Return'], mode='lines', name='SPY5 Total Return',line=dict(color="#4472C4",width=2))
    trace3 = go.Scatter(x=df['Date'], y=df['UST_Total_Return'], mode='lines', name='US Treasury Total Return')
    layout_performance = go.Layout(title='Performance Overview: SPY5 vs. SPX',
...
# VaR with a second y-axis
    ...
    layout_prices_and_var = go.Layout(title='Inside the SPY5 Model: Prices, Moving Averages and VaR',
    ...
    # Drawdown comparison 
    trace_dd_spy3 = go.Scatter(x=df['Date'], y=df['DD_SPY3'], mode='lines', name='Max Drawdown SPY5',line=dict(color="#4472C4",width=2))
    trace_dd_spy = go.Scatter(x=df['Date'], y=df['DD_SPY'], mode='lines', name='Max Drawdown SPY',line=dict(color="grey",width=2))
    layout_dd = go.Layout(title='Maximum Drawdown Comparison: SPY5 vs. SPY', 
    ...
    layout_roll_perf = go.Layout(title='5y Rolling Annual Performance: SPY5 vs. SPX', 
                             xaxis=dict(title='Date'), 
    ...
    layout_calmar = go.Layout(title='Rolling Calmar Ratios for SPY & SPY5', xaxis=dict(title='Date'), yaxis=dict(title='calmar-ratio-graph'), legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1), showlegend=True)
...
if __name__ == '__main__':
    app.run_server(debug=False, host='0.0.0.0', port=int(os.environ.get('PORT', 8060)))
    server = app.server
